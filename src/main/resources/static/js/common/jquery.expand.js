...
// 重写jquery的ajax方法
    $.ajax = function (opt) {
        ...
        if (opt.data == undefined) {
            opt.data = "";
        } else if (typeof(opt.data) == "object" && Object.prototype.toString.call(opt.data).toLowerCase() == "[object object]" && !opt.data.length) {
...
(function ($, document) {
    ...
    $.extend({
        ...
        /**
         * 功能：当鼠标指针Hover在弹窗内部区域时，无法触发“页面滚动条”。当鼠标指针Hover在弹窗以外的区域时，仅能触发“页面滚动条”。
         * @param id1:弹窗最外层id
         * @param id2:弹窗里面滑轮外的id2
         * @param id3:弹窗里面滑轮所在框的高度，有时就是id2
         * @param aveHeight:滑轮上下一次平均高度
         * 例子是加入case弹窗
         * */
        _changeScroll: function (id1, id2, id3, aveHeight) {
            $('body').on("mousewheel DOMMouseScroll", function (e) {
                var target = $(e.target);   //获取当前鼠标目标
                if (target.closest(id1).length === 1) {
                    //滑轮高度值
                    var scrollHeight = $(id2).scrollTop();
                    var divTop = $(id2).offset().top;//252
                    var divHeight = $(id3).height() + 50;//782
//                    var scrollHeight = $("#batchCollectFolder .yy_popFavList").scrollTop();
//                    var divTop = $("#batchCollectFolder .yy_popFavList").offset().top;//252
//                    var divHeight = $("#batchCollectFolder .orgListDiv").height()+50;//782
                    var maxTop = Math.abs(divHeight - divTop) + 100;//630
                    var ave = aveHeight || 30;
                    var top = 0;
                    var delta = (e.originalEvent.wheelDelta && (e.originalEvent.wheelDelta > 0 ? 1 : -1)) ||  // chrome & ie
                        (e.originalEvent.detail && (e.originalEvent.detail > 0 ? -1 : 1));              // firefox
                    if (delta > 0) {
                        // 向上滚
                        top = scrollHeight - ave;
                        top = top > 0 ? top : 0;
                    } else if (delta < 0) {
                        // 向下滚
                        top = scrollHeight + ave;
...
//            $('body').css('overflow', '');
        ...
        _bottomFix: function () {
            ...
            if (i > 0) {
                if (i < objFoot.innerHeight()) {
                    ...
                    if (i > objFoot.innerHeight()) {
                        //高度大于"底部高度"，可以使用绝对定位。
                        strCssText = "width:100%;position:absolute;top:100%;z-index:-1;margin-top:-" + objFoot.innerHeight() + "px;";
                    }
                } else {
        ...
        _linkHref: function (loginRole, viewerRole, viewerId, $caseId, passKey, agentPasskey) {
            ...
            switch (interactiveRole.toLowerCase()) {
                ...
                case 'managerspy':
                    ...
                    window.open(WWW_ADDR_DOMAIN + '/spy/spyinfo.php' + spyInfo);
                    break;
                case 'spymanager':
                case 'managermanager':
                    if ($._trim(viewerId) != '' && $._trim(passKey) != '') {
                        ...
                        window.open(WWW_ADDR_DOMAIN + '/manager/cv.php?act=showCv&managerId=' + viewerId + '&caseid=' + $caseId + '&isenglish=0&passkey=' + passKey + '&agentPasskey=' + agentPasskey);
                    } else {
                        window.open(WWW_ADDR_DOMAIN + '/manager/managerprofile.php?act=showcv&isenglish=0&type=2');
                    }
                    break;
                default:
                    return false;
    ...
    $.fn.extend({
        ...
        _imgPop: function (title, src) {
            ...
            var mouseOverSrc = WWW_ADDR_DOMAIN + "/images/closeIconMouseOver.png";
            var mouseOutSrc = WWW_ADDR_DOMAIN + "/images/closeIconMouseOut.png";
...
var RegExpObj = {
    ...
    phoneRich: function (str) {
        return /(^(([+]{0,1}\d{2,4}|\d{2,4})-?)?1[3456789]\d{9}$)/.test(str);
    },
    //国号+电话/传真
